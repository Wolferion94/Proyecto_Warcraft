...
class analytics:
    def __init__(self, df, colors=list()):
        self.colors = colors
        self.df = df
    ...
    def caract_num(self, col_list):
        ...
        for i in range(len(col_list)):
            ...
            if len(self.colors)>=i+1:
                params_plot['color'] = self.colors[i]
            self.df[col_list[i]].plot(**params_plot)
        ...
        for i in range(len(col_list)):
            ...
            if len(self.colors)>=i+1:
                params_boxplot['color'] = self.colors[i]
                params_boxplot['flierprops'] = {
                    "marker": "d", 
                    "markerfacecolor": self.colors[i], 
                    "markersize": 8,
                    "markeredgecolor": self.colors[i]
                }
            self.df.boxplot(**params_boxplot)
        plt.show()
    ...
    def caract_obj(self, col_list):
        ...
        for i in range(len(col_list)):
            ...
            if len(self.colors)>=i+1:
                params_bar['color'] = self.colors[i]
            self.df[col_list[i]].value_counts().plot(**params_bar)
        plt.show()
...
#Esta función se encarga de evaluar todas las métricas que utilizara el modelo
    ...
    def calculate_metrics(self,model,type = 'test'):
